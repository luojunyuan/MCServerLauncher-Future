...
public sealed class CreateTerrariaInstanceProvider : CreateInstanceProviderPage
{
    ...
    public CreateTerrariaInstanceProvider(CreateInstancePage owner, CreateInstanceSession session) : base(owner, session)
    {
    ...
    protected override async Task FinishAsync()
    {
        ...
        var setting = new InstanceFactorySetting
        {
            ...
            Target = Path.GetFileName(archive), TargetType = TargetType.Script,
            InstanceType = InstanceType.Terraria, Version = string.Empty
        };
        if (!await ConfirmAsync(BuildConfirmation(Texts["CreateTerrariaServerInstance"], name, ("Archive", archive)))) return;
